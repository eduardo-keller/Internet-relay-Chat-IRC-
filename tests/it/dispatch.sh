...
# 6. THIS ONE IS EXPECTED TO CHANGE. JOIN is not in the table yet, so it is
#    answered as unknown. The day the DOMAIN block in src/CommandTable.cpp is
#    uncommented, JOIN becomes a known command and this turns into 451 — and
#    this line failing is the reminder to update it.
check "JOIN ainda desconhecido (vira 451 quando o DOMAIN entrar na tabela)" \
	":ircserv 421 * JOIN :Unknown command" "$(talk 'JOIN #x\r\n')"
